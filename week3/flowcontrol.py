...
if pathChoice == "1":
    print(
        f"This astroid field is much denser than expected... {ship} is really taking a beating! Should we go left or right?")
    ...
    while True:
        ...
        if astroidField == "1":
            print(
                f"Uh oh. You turned directly into a massive astroid that ripped the hull of {ship} apart. Captain {name} was sucked into the vacuum of space. You are dead.")
            exit()
...
if pathChoice == "2":
    print(
        f"This void is much darker than expected... {ship}'s navigation systems can't tell up from down! Should we follow our head or our heart?")
    ...
    while True:
        ...
        if deepVoid == "1":
            print(
                f"Uh oh. You decided to follow your head, and it lead you directly into a hostile aliens dinner party. He was not happy. You got vaporized, and you are dead.")
            exit()
        # Deep void success (heart) - allows you to move on to the ship wash
        elif deepVoid == "2":
            print(
                f"Awesome job Captain {name}. Always best to follow your heart. Now Lets go wash this ship a well deserved wash!")
            time.sleep(3)
        # Ship Wash Ending - allows you to choose your ships scent
        print(
            "You Make your way to the shipwash after a long, hard day of space exploration, would you like to use 1)Lemon, or 2)New Ship Scent?")
        scent = input("--> ")
        if scent == "1":
            print(f"You chose the lemon scent. You always loved the clean, refreshing smell of lemons")
            exit()
